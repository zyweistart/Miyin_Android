...
public class HanziToPinyin {
	...
	public String getTokensForDialerSearch(final String input,
		...
		for (int i = 0; i < inputLength; i++) {
			...
			if (character == '-' || character == ',') {
				...
				// Comment out. Do not cover unknown characters SINCE they can
				// not be input.
				...
				if (t.type == Token.PINYIN) {
					...
					// Comment out. Do not cover special characters SINCE they
					// can not be input.
					...
					// caseTypePre = Token.FIRSTCASE;
					// }else{
					// caseTypeCurr = (character>='A' &&
					// character<='Z')?Token.UPPERCASE:Token.LOWERCASE;
					// if(caseTypePre == Token.LOWERCASE && caseTypeCurr ==
					// Token.UPPERCASE){
					// addToken(subString, tokens, tokenType);
					// addOffsets(subStrOffset, shortSubStrOffset);
					// }
			...
			// IF the name string is too long, cut it off to meet the storage
			// request of dialer search.
			if (mPos > 127)
				break;
		}
		if (subString.length() > 0) {
	...
	private void addOffsets(final StringBuilder sb,
	...
	private void addSubString(final ArrayList<Token> tokens,
			final ArrayList<String> shortSubStrOffset, StringBuilder subStrSet,
			StringBuilder offsets) {
