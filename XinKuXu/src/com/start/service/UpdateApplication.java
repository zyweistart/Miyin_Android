...
public class UpdateApplication {
	...
	public void startCheck(final boolean backgroundCheckFlag) {
		if (NetConnectManager.isNetworkConnected(currentActivity)) {
			HttpServer hServer=new HttpServer(Constant.URL.VersionCheck, mHandlerContext);
			...
			hServer.get(new HttpRunnable() {
				...
				public void run(Response response) throws AppException {
					try {
						final Integer minver = Integer.parseInt(String.valueOf(response.getData("lowestAvailable")));
						final Integer nowver =Integer.parseInt(String.valueOf(response.getData("version")));
						final String description = String.valueOf(response.getData("versionTips"));
						final String url = String.valueOf(response.getData("apkUrl"));
	...
	private class DownloadAppTask extends AsyncTask<String, Float, File> {
		...
		protected File doInBackground(String... params) {
			...
			// 主目录创建
			File dirFile = new File(BaseContext.getInstance().getStorageDirectory(Constant.TEMPDIRECTORY));
			if (!dirFile.exists()) {
				dirFile.mkdirs();
			}
